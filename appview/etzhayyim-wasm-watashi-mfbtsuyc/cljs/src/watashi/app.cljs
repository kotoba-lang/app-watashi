(ns watashi.app
  "etzhayyim-wasm-watashi-mfbtsuyc appview — reagent + re-frame, view built
  ...
  Svelte page never called the dispatcher Worker (`../src/app.ts`) — it was
  ...
  documented dispatcher Worker) is untouched. The one other TypeScript file
  that lived under the deleted `svelte/` tree —
  `svelte/src/routes/xrpc/[...path]/+server.ts`, a SvelteKit server route
  that proxied XRPC calls to AGENTGATEWAY_MCP_ROUTER_URL — was backend/XRPC
  logic, not frontend markup, so it was moved rather than deleted; see
  `../src/xrpc-proxy.ts` for its unmodified body and a note on why it is not
...
(def default-db
  {:title "Watashi Mfbtsuyc"
   ...
   :name "etzhayyim-wasm-watashi-mfbtsuyc"
   :kind "appview"
   :route-count 1
   :routes ["mfbtsuyc.etzhayyim.com/*"]
   :vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_ACTOR_HANDLE" "APP_CAPABILITIES"
          "APP_DESCRIPTION" "APP_DISPLAY_NAME" "APP_EMBED_URL" "APP_FRAMEWORK"
          "APP_NANOID" "APP_PERFORMER_TYPE" "APP_UI_TYPE"]
   :xrpc? true
   :relative-path "appview/etzhayyim-wasm-watashi-mfbtsuyc/cljs/src/watashi/app.cljs"})
